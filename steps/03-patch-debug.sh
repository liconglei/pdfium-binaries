...
apply_patch() {
  local FILE="$1"
  local DIR="${2:-.}"
  echo "Applying patch: $FILE to $DIR"
...
echo "Patches directory: $PATCHES"
echo "Source directory: $SOURCE"

[ "$BUILD_TYPE" == "shared" ] && [ "$OS" != "emscripten" ] && apply_patch "$PATCHES/shared_library.patch"
echo "After shared_library patch"
apply_patch "$PATCHES/common/implementation_config.patch"
echo "After implementation_config patch"
cat BUILD.gn | grep -A3 "pdfium_implementation_config"
echo "---"
...
case "$OS" in
  android)
    echo "Applying fpdfsdk patch for android"
    apply_patch "$PATCHES/common/fpdfsdk.patch"
    cat fpdfsdk/BUILD.gn | grep -A5 'configs +='
